classdef ParticlePyramid < Particle
    ...
    methods
        function obj = ParticlePyramid(v,a,psi,nm,np)
            ...
            Check.samesize('v, a, psi must be of size 1',v,a,psi,1)

            obj.pyr = Pyramid2(v,a,psi);
            obj.nm = nm;
            obj.np = np;
        end
        ...
        function disp(bead)
            ...
            disp(['<a href="matlab:help ParticlePyramid">ParticlePyramid</a> (a=' num2str(bead.pyr.a) ', psi=' num2str(bead.pyr.psi) ', nm=' num2str(bead.nm) ', np=' num2str(bead.np) ') : x=' num2str(bead.pyr.v.X) ' y=' num2str(bead.pyr.v.Y) ' z=' num2str(bead.pyr.v.Z) ' Vx=' num2str(bead.pyr.v.Vx) ' Vy=' num2str(bead.pyr.v.Vy) ' Vz=' num2str(bead.pyr.v.Vz)]);
        end
        function bead_t = translate(bead,dp)
            ...
            bead_t = bead;
            bead_t.pyr = bead_t.pyr.translate(dp);
        end
        function bead_r = xrotation(bead,phi)
            ...
            bead_r = bead;
            bead_r.pyr = bead_r.pyr.xrotation(phi);
        end
        function bead_r = yrotation(bead,phi)
            ...
            bead_r = bead;
            bead_r.pyr = bead_r.pyr.yrotation(phi);
        end
        function bead_r = zrotation(bead,phi)
            ...
            bead_r = bead;
            bead_r.pyr = bead_r.pyr.zrotation(phi);
        end
        ...
        function p = barycenter(bead)
            ...
            p = Point(bead.pyr.v.X,bead.pyr.v.Y,bead.pyr.v.Z);
        end
        function r_vec = scattering(bead,r,err,N)
            ...
            [r_vec(1).r,r_vec(1).t] = r.snellslaw(bead.pyr,bead.nm,bead.np,1);

            [r_vec(2).r,r_vec(2).t] = r_vec(1).t.snellslaw(bead.pyr,bead.np,bead.nm,2);
          
            for n = 2:1:N
                [r_vec(n+1).r,r_vec(n+1).t] = r_vec(n).r.snellslaw(bead.pyr,bead.np,bead.nm,2);
        ...
        function f = force(bead,r,err,N)
            ...
            f = fi-fe;
            f.X = bead.pyr.v.X.*ones(size(f));
            f.Y = bead.pyr.v.Y.*ones(size(f));
            f.Z = bead.pyr.v.Z.*ones(size(f));
        end
        function T = torque(bead,r,err,N)
            ...
            T = Ti - Te;
            T.X = bead.pyr.v.X.*ones(size(T));
            T.Y = bead.pyr.v.Y.*ones(size(T));
            T.Z = bead.pyr.v.Z.*ones(size(T));
        end
    end
end