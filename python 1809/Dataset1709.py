...
class DS(Dataset):  #inheritance from pytorch Dataset: length __len__() return __getitem__(idx)
    def __init__(self, data_dir, label_file, transform=None):
        ...
        # 预加载所有图像到内存 - 这是关键部分
        self.loaded_images = []  # 存储所有加载的图像
        self.loaded_labels = []  # 存储所有加载的标签
        ...
        for sample_id in self.sample_ids:
            ...
            for suffix in ["xy", "yz", "xz"]:
                ...
                if self.transform:
                    img = self.transform(img)  # 形状: [1, H, W] (单通道)
                img = img * 51  # 数值缩放
                image_set.append(img)

            # 合并为三通道图像
            combined_image = torch.cat(image_set, dim=0)  # 形状: [3, H, W]
    ...
    def normalize_labels(self, label_min=None, label_max=None, label_range=None):
            if label_min is not None and label_range is not None:
                self.normalized_labels = (self.loaded_labels - label_min) / label_range
...
if __name__ == "__main__":
    ...
    # verify [0, 1]
    print("\nVerifying all labels are in [0, 1] range:")
    ...
    def check_range(dataset, name):
        ...
        print(f"{name}: min={min_val:.6f}, max={max_val:.6f}, in_range={min_val >= 0 and max_val <= 1}")
